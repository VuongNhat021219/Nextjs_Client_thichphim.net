...
import { getOnePhimHaiHuoc, getAllPhimHaiHuoc } from "@/pages/api/getData";
...
export default function PhimHaiHuoc({ data, allTrendMovieDatas }) {
  return (
    <div className={`app`}>
      <Layout>
                ...
                <Row>
                  <Col>
                    {data === undefined ? (
                      ...
                      <MovieList
                        title="Danh Sách Phim Hài Hay Nhất"
...
export async function getStaticProps({ params }) {
  const { page } = params;
  const data = await getOnePhimHaiHuoc(page);
...
export async function getStaticPaths() {
  const data = await getAllPhimHaiHuoc();
