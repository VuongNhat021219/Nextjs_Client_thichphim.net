...
import { getOnePhimKhoaHoc, getAllPhimKhoaHoc } from "@/pages/api/getData";
...
export default function PhimKhoaHoc({ data, allTrendMovieDatas }) {
  return (
    <div className={`app`}>
      <Layout>
...
export async function getStaticProps({ params }) {
  const { page } = params;
  const data = await getOnePhimKhoaHoc(page);
...
export async function getStaticPaths() {
  const data = await getAllPhimKhoaHoc();
